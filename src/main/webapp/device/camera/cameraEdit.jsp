...
<body>
	...
	<div class="easyui-panel"
		...
		<table width="600px">
		   		...
		   		<tr>
		   			<td>
		   				<input class="easyui-numberbox" id="webPort" name="webPort" value="80" style="width: 250px" data-options="label:'web端口:',required:true">
		   			</td>
		   			<td>
		   		...
		   		<tr>
		   			...
		   			<td>
		   				<input class="easyui-numberbox" id="recorderPort" name="recorderPort" value="80" style="width: 250px" data-options="label:'录像机端口:',required:true">
		   			</td>
		   		</tr>
		   		<tr>
		   			<td>
	...
	var cameraNumExist = false;
		$(function() {
			...
			$.ajax( {
			    ...
			    data:{
			    	'sqlId':'alarm-camera-getCameraById',
			    	'cameraId':'<%=cameraId%>'
			    },
			    type:'post',
			    async:false,
			    ...
			    success:function(data) {
			    	if(data!=null && data.length > 0){
			    		var cameraObj = data[0];
			    		$("#classId").textbox('setValue', cameraObj.class_id);
			    		$("#cameraNum").textbox('setValue', cameraObj.camera_num);
			    		$("#posDesc").textbox('setValue', cameraObj.pos_desc);
			    		$("#longitude").textbox('setValue', cameraObj.longitude);
			    		$("#latitude").textbox('setValue', cameraObj.latitude);
			    		$("#ip").textbox('setValue', cameraObj.ip);
			    		$("#webPort").textbox('setValue', cameraObj.web_port);
			    		$("#devicePort").textbox('setValue', cameraObj.device_port);
			    		$("#cameraUserName").textbox('setValue', cameraObj.camera_user_name);
			    		$("#cameraPassword").textbox('setValue', cameraObj.camera_password);
			    		$("#recorderIp").textbox('setValue', cameraObj.recorder_ip);
			    		$("#recorderPort").textbox('setValue', cameraObj.recorder_port);
			    		$("#recorderUserName").textbox('setValue', cameraObj.recorder_user_name);
			    		$("#recorderPassword").textbox('setValue', cameraObj.recorder_password);
			    		$("#channelId").textbox('setValue', cameraObj.channel_id);
			    	}
			    },
			    error : function(data) {
		...
		function okResponse(){
			...
			// 提交保存
			$.ajax( {
			    url:'<%=path%>/camera/mgr/updateCamera.do',
			    data:{
			    	'cameraId':'<%=cameraId%>',
			    	'classId':$("#classId").val(),
