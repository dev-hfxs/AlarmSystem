...
var lastLoadIndex = 0;
	...
	function showButtons(val,row){
		...
		if(row.status == 'I'){
			columnItem = columnItem + '<span><a href="javascript:void(0)" class="easyui-linkbutton" onclick="doReconfirm(\''+val+'\')" style="width:80px;">重新确警</a></span>&nbsp;&nbsp;';
		}
		return columnItem;
	}
	...
	function doReconfirm(alarmId){
		var content = '<iframe name="popContent" id="popContent" src="<%=path%>/alarm/alarmReconfirm.jsp?id=' + alarmId + '" width="100%" height="100%" frameborder="0" scrolling="no"></iframe>';
		var boarddiv = '<div id="msgwindow" title="重新确认报警" style="overflow:hidden;"></div>'// style="overflow:hidden;"可以去掉滚动条
		$(document.body).append(boarddiv);
		var win = $('#msgwindow').dialog({
			content : content,
			width : '640',
